...
async def find_api_results(message: Message, state: FSMContext) -> Any:
    ...
    if user_data.get("request") == "low":
        ...
        api_result: Optional[list, str] = APIResult.get_custom_tickets_short(
            **user_data
        )
    else:
        api_result: Optional[list, str] = APIResult.get_direct_ticket(**user_data)
    if api_result is None:
        HistoryMethods.create_history(
            search_params=json.dumps(user_data),
    ...
    if api_result is None or api_result == [] or api_result == {}:
        if user_data.get("request") == "low":
            await message.answer(
                text="К сожалению, по вашему запросу результаты не найдены.\n"
                "Хотите поменять даты вылета?",
                reply_markup=change_date_kb(),
            )
        ...
        if user_data.get("request") == "low":
            await message.answer(
                text=f"По вашему запросу найдено {len(api_result)} вариантов.\n"
                f"Какое количество ответов вывести?",
            ...
            if (
                user_data.get("return_at", 0)
                and user_data.get("group_by") == "departure_at"
            ):
                text: str = (
                    "Туда/   Обратно/  Цена за 2б./   Время туда+обратно\n" + api_result
                )
            elif (
                user_data.get("return_at", 0)
                and user_data.get("group_by") == "return_at"
            ):
                text: str = (
                    "Обратно/   Туда/  Цена за 2б./   Время туда+обратно\n" + api_result
                )
            else:
                text: str = "Дата/    Цена/   Общее время в пути\n" + api_result
            await message.answer(text=text)
            await message.answer(
                text="Для заказа и получения деталей перейдите по ссылке:",
...
async def low_command(message: Message, state: FSMContext) -> None:
    ...
    """
    await message.answer(
        text="Для поиска самого дешёвого билета введите город отправления.\n"
        "Например: Москва",
        reply_markup=ReplyKeyboardRemove(),
    )
...
async def custom_command(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        text="Для поиска дешёвых билетов за каждый день периода введите город отправления.\n"
        "Например: Москва",
        reply_markup=ReplyKeyboardRemove(),
    )
    await state.set_state(TicketState.origin_city)
...
async def high_command(message: Message, state: FSMContext) -> None:
    ...
    """
    await message.answer(
        text="Для поиска прямого билета введите город отправления.\n"
        "Например: Москва",
        reply_markup=ReplyKeyboardRemove(),
    )
...
async def get_origin_city(message: Message, state: FSMContext) -> None:
    ...
    try:
        ...
        if origin_code is None:
            await message.answer(
                text="Ошибка! Некорректно введен город отправления\n"
                "Попробуйте снова:"
            )
        else:
            ...
            await message.answer(
                text="А теперь введите город назначения:",
                reply_markup=ReplyKeyboardRemove(),
            )
            await state.set_state(TicketState.destination_city)
    except Exception:
        await message.answer(
            text="Ошибка! Некорректно введен город.\n" "Попробуйте снова:"
        )
...
async def define_cities(message: Message, state: FSMContext) -> None:
    ...
    try:
        ...
        if destination_code is None:
            await message.answer(
                text="Ошибка! Некорректно введен город назначения.\n"
                "Попробуйте снова:"
            )
        else:
            ...
            if user_data.get("request") in ["high", "low"]:
                await message.answer(
                    text="Отлично! Перейдем к дате отправления.\n"
                ...
                await message.answer(
                    text=text, reply_markup=await start_calendar_kb(message.from_user)
                )
    ...
    if selected:
        if user_data.get("one_way", None) is None:
            if user_data.get("date_type") == "exact_date":
                ...
                await callback_query.message.answer(
                    text=f'Вы выбрали месяц {date.strftime("%m.%Y")}.\n'
                )
            await callback_query.message.answer(
                text=f"Вам нужен обратный билет?\n" f"Выберите да или нет...",
                reply_markup=y_n_kb(),
            )
        else:
            if user_data.get("date_type") == "exact_date":
                ...
                if date < departure_at_date:
                    await callback_query.message.answer(
                        text="Дата возвращения не может быть раньше даты вылета туда.\n"
    ...
    if callback.data == "Да":
        ...
        if user_data.get("request") in ["low", "high"]:
            await callback.message.answer(
                text="Вы хотите указать точную дату обратного вылета или только месяц?",
                reply_markup=date_choice_kb(),
            )
        else:
            await callback.message.answer(
                text="Введите любое число месяца обратного вылета...",
                reply_markup=await start_calendar_kb(callback.from_user),
            )

    else:
        api_result: Optional[list, str] = await find_api_results(
...
async def counts_to_show(message: Message, state: FSMContext) -> None:
    ...
    try:
        ...
        if 1 <= count_to_show <= len(api_result):
            await answer_to_user(
                message,
                state,
                api_result,
                default_num=count_to_show,
                not_defined_by_user=False,
            )
        else:
            await message.answer(
                text=f"Ошибка! Введите число в допустимых пределах, максимально - {len(api_result)}"
            )
    except TypeError:
        await message.answer(
            text=f"Ошибка! Введите корректное число, максимально - {len(api_result)}"
        )
...
async def change_dates(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if callback.data == "Поменять":
        ...
        await callback.message.answer(
            text="Вы хотите указать точную дату вылета или только месяц?",
            reply_markup=date_choice_kb(),
        )
    else:
        await callback.message.answer(
            text="Благодарю за обращение. Завершаю обработку запроса...",
...
async def date_type_choice(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if action == "exact_date":
        await callback.message.answer(
            text="Выберите точную дату:",
            reply_markup=await start_calendar_kb(callback.from_user),
        )
    elif action == "month":
        await callback.message.answer(
            text="Выберите любой день месяца вылета",
            reply_markup=await start_calendar_kb(callback.from_user),
        )

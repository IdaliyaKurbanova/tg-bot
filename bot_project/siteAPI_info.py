...
def get_fastest_ticket(response):
    ...
    """
    list_of_durations = [
        (value["duration"] // 60 + value["duration"] % 60 * 0.01)
...
class APIResult:
    ...
    def get_cheapest_ticket(**kwargs) -> list[str]:
        ...
        url = (
            f"https://api.travelpayouts.com/aviasales/v3/prices_for_dates?{data_str}"
        ...
        if response:
            if response[0].get("return_at", 0):
                selected_data = [
                    f"Цена за 2 билета: {flight['price']}\n "
    ...
    def get_direct_ticket(**kwargs) -> list[str]:
        ...
        data_str = "&".join(data_list)
        url = (
            f"http://api.travelpayouts.com/v1/prices/direct?{data_str}"
    ...
    def get_custom_tickets_short(**kwargs) -> str:
        ...
        """
        data_list = [
            f"{name}={value}" for name, value in kwargs.items() if name != "one_way"
        ]
        data_str = "&".join(data_list)
        url = (
            f"https://api.travelpayouts.com/aviasales/v3/grouped_prices?{data_str}"
            f"&token={config.API_KEY}"
        )
        response = json.loads(requests.request("GET", url).text)["data"]
        ...
        if response:
            ...
            selected_data[min_price] = (
                selected_data[min_price] + "   * \n- один из самых дешёвых"
            )
            selected_data[min_duration] = (
